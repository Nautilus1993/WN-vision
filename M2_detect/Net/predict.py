...
import cv2
import numpy as np
from data.center_calculate import geo_calculate, moon_calculate, quaternion2rot
...
def net_pre(model,media_path,resize):
    ...
    # 读取图片
    frame_old = cv2.imread(media_path)
    # frame_old = cv2.resize(frame_old, (resize, resize))
    # frame = resize_and_pad(media_path)
    frame = frame_old
    scale_img = frame.shape[0] / resize
    # print(frame.shape[0],frame.shape[1])
    frame = cv2.resize(frame, (resize, int(resize/frame.shape[1]*frame.shape[0])))
    bbox_center = None
    ellipse_center = None
    obj_core = None
    rad_ = None
    ...
    for box in boxes.data:
        ...
        # l,t,r,b = resize/2 + (l-resize/2)*4,resize/2 + (t-resize/2)*4,resize/2 + (r-resize/2)*4,resize/2 + (b-resize/2)*4
        conf, id = box[4:] # confidence, class
        bbox_center = np.array([(l+r)/2*scale_img, (t+b)/2*scale_img])
        id = int(id)
        # 绘制框
        cv2.rectangle(frame, (int(l),int(t)), (int(r),int(b)), class_color[id], 2)
        # 绘制类别+置信度（格式：98.1%）
        cv2.putText(frame, f"{objs_labels[id]} {conf*100:.1f}%", (int(l),int(t-10)), cv2.FONT_HERSHEY_SIMPLEX, 1, class_color[id], 2)
    # # if id == 0:
    # # 转换颜色空间
    # image = frame[t:b,l:r]
    ...
    for keypoint in keypoints.data:
        for i in range(len(keypoint)):
            if i == 0:
                x,y,c = keypoint[i]
                # x,y = resize/2 + (x-resize/2)*4, resize/2 + (y-resize/2)*4,
        ...
        if len(keypoint) >= 2:
            ...
            x1,y1,c1 = keypoint[0]
            x2,y2,c2 = keypoint[1]
            # x1, y1, x2, y2 = resize/2 + (x1-resize/2)*4, resize/2 + (y1-resize/2)*4, resize/2 + (x2-resize/2)*4, resize/2 + (y2-resize/2)*4,
            center_x, center_y = (x1+x2)/2, (y1+y2)/2
            cv2.arrowedLine(frame_old, (int(x2),int(y2)), (int(center_x), int(center_y)), (255,0,255), 4, line_type=cv2.LINE_AA, tipLength=0.1)
    ...
    # save image
    if not os.path.exists(r"Net/result/{}".format(media_path.split('/')[-2]+str(resize))):
        os.mkdir("Net/result/{}".format(media_path.split('/')[-2]+str(resize)))
        os.mkdir("Net/bbox/{}".format(media_path.split('/')[-2]+str(resize)))
    cv2.imwrite("Net/result/{}/{}".format(media_path.split('/')[-2]+str(resize),media_path.split('/')[-1]), frame)
    if t is None:
        l,r,t,b = 0,int(resize),0,int(resize)
    cv2.imwrite(r"Net/bbox/{}/{}".format(media_path.split('/')[-2]+str(resize),media_path.split('/')[-1]), frame[int(t):int(b),int(l):int(r)])
...
def plot_err_pixle(time_name,x_lim,suc_err_net_m, suc_err_ed_m,type=None):
    ...
    a = median_filter(np.array(suc_err_net_m)[:, 0], 1)
    b = median_filter(np.array(suc_err_net_m)[:, 1], 1)
    c = median_filter(np.array(suc_err_net_m)[:, 2], 1)
    d = median_filter(np.array(suc_err_ed_m)[:, 0], 1)
    e = median_filter(np.array(suc_err_ed_m)[:, 1], 1)
    ...
    ax[0].plot(x_lim,np.array(suc_err_net_m)[:, 2], label='2')
    ...
    # ax[1].plot(x_lim,np.array(suc_err_net_m)[:, 2], label='2')
    # # ax[0].plot(x_lim,np.array(suc_err_ed_m)[:, 0], label='Edellipse_core_err')
...
def resize_and_pad(image_path):
    ...
    if img.shape[0] != 2048 or img.shape[1] != 2048:
        raise ValueError("输入图像尺寸不是2048x2048")

    # 缩小4倍（512x512）
    scaled_img = cv2.resize(img, None, fx=0.25, fy=0.25, interpolation=cv2.INTER_AREA)

    # 创建全黑的2048x2048画布
    padded_img = np.zeros((2048, 2048, 3), dtype=np.uint8)

    # 计算填充位置（居中）
    y_start = (2048 - scaled_img.shape[0]) // 2
    x_start = (2048 - scaled_img.shape[1]) // 2

    # 将缩小后的图像放入画布中央
    padded_img[y_start:y_start + scaled_img.shape[0],
    x_start:x_start + scaled_img.shape[1]] = scaled_img
    return padded_img
...
if __name__ == '__main__':
    ...
    weight_path = para.modelfile
    model = YOLO(weight_path)
    media_dir = para.testdata
    ...
    for media_path in media_:
        ...
        if id is None:
            ...
            def obt_err_net_ed(r, core_gt):
                ...
                if r < 4000:
                    xyz_moon, lighttimem = spice.spkpos(para.sat_id_str, timestamps, 'J2000', 'NONE', 'Moon')
